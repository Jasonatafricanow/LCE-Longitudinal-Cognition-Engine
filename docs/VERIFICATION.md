...
`requirements-verification.txt` is the public verification toolchain, currently pinned to exact versions. A reviewer should not substitute a different mypy/Ruff/pytest version and then describe the result as the same gate without recording that difference.
...
Those require separate public replication and external adversarial audit evidence. See `docs/PUBLIC_EVIDENCE_MATRIX.md` and `docs/audit/` when those surfaces are available.
